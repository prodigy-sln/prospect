...
2. Locate its tests via the spec folder's `test-map.md` (a scenario maps to
   one or more tests). Verify the map itself: every named test exists in
   its named file — a scenario with zero mapped tests, or a map entry
   naming a test that does not exist, is a Major finding.
