...
| D19 | **No spec references in code, ever**: test names are purely behavioral; the scenario↔test mapping is a spec-folder artifact (`test-map.md`) written by the test author and consumed by the coverage review; no provenance comments in test files | Scenario IDs are unique only per spec and accumulate as noise in shared test files; the original reasoning is consolidated into `docs/` which every agent reads, and git history covers archaeology — an in-code ID scheme duplicates both, badly |
| D20 | **Two-phase completion**: Publish (validation PASS → docs consolidation, `specs/REGISTRY.md` append, PR; spec stays in `specs/active/` through review) and Finalize (PR approved → remove folder, squash-merge; `main` never carries the spec). Default `spec-disposal: delete`; `archive` mode (move at Publish to `specs/archive/YYYY/`, rolling `retention`, pruned at each Publish) for regulated projects or strict branch protection | The spec lifecycle ends at merge, not validation: review fixes need the spec on disk (no git recovery in the normal loop); deletion on the branch pre-approval breaks review visibility, post-approval may need one re-approval — documented, with archive mode as the zero-post-approval alternative; REGISTRY.md keeps existence squash- and prune-proof |
