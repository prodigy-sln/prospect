...
Present the Drivers, Boundaries, and Decisions tables to the user.

- `rigor: high` (no `/sdd-discuss` step ahead): request one review round
  from the `persona-architect` agent — Mode B, since a draft
  `architecture.md` now exists — before marking the architecture
  binding. Fold its verdicts into the draft (or record why a blocker was
  overridden) before committing.
- `rigor: xhigh` / `max`: skip the inline persona round. `/sdd-discuss`
  runs the full persona panel against this draft next, so a solo review
...
```
Architecture drafted and committed — safe to /clear.
Next: /sdd-discuss (xhigh/max) or /sdd-tasks.
```
