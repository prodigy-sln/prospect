...
`feat:` commits, ticking each task `- [x]`, and scoped test runs via
`test-map.md`; one refactor pass per phase. Test ownership and arbitration
apply. Wire each check where the decision record places it (project checks
into the gate only via the project-level gate policy). Run
`scripts/sdd-gate.*` to exit 0.
